# Configuring LDAP integration in Universal Database
...
The following settings allow you to specify LDAP attributes for user display names, email addresses, and other attributes. These attributes are automatically updated from the LDAP server every time the user logs in.
...
If you want to limit access to Universal Database based on LDAP groups, add the following lines to the `udb.conf` file:
...
| Option | Description |
| --- | --- |
| --ldap-required-group | This option specifies the list of common names (CN) of the LDAP group(s) that contain users who are allowed to access Universal Database. This option should be set to a comma-separated list of CN values. Note that you should not include the full distinguished name (DN) of the group(s) here. For example, if your LDAP group is named udb_users and its DN is cn=udb_users,ou=groups,dc=mydomain,dc=com, you would simply specify udb_users as the value for this option. |
...
## LDAP Groups & Role mapping

Universal Database LDAP integration provides a means to map LDAP groups to specific application roles and permissions. Here is a list of configurable options to define the mapping between roles and groups. Each option can be defined multiple times to assign multiple groups to the same role.

| Option | Description |
| --- | --- |
|--ldap-admin-group | list of common name (CN) of the group(s) containing administrator. This option could be define multiple time to assign multiple group. |
|--ldap-subnet-mgmt-group | list of common name (CN) of the group(s) containing Subnet Managers. |
|--ldap-dnszone-mgmt-group | list of CN of the group(s) containing DNS Zone Managers. |
|--ldap-user-group | list of common name (CN) of the group(s) containing Users. Not cn=groupname or the full DN. |
|--ldap-guest-group | list of CN of the group(s) containing Guests. Not cn=groupname or the full DN. |

## Advance Settings

Here are more details about the remaining options used to configure the LDAP integration of Universal Database. These options are set with sensible default values that should fit the needs of most environments.
